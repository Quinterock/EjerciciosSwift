...
class SongsLoader {
    ...
    private func songsUnderThreeMinutes() -> [Song] {
        return songs.filter { $0.basicInfo.durationMs < 180000 }
    }
    
    //8 Listar canciones donde las canciones de diferentes duraciones estén mezcladas de manera aleatoria
    
    private func getRandimizedSongs() -> [Song] {
        return songs.shuffled()
    }
    
    //6 Contar cuántas canciones hay por género
    
    private func countSongsByGenre() -> [String: Int] {
        return songs.reduce(into: [String: Int]()) { counts, song in
            let genre = song.basicInfo.genre
